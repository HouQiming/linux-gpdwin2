...
static void stream_timeout_function(struct timer_list* stream_timeout_timer)
{
	struct dwc3_ep *dep = container_of(stream_timeout_timer, struct dwc3_ep, stream_timeout_timer);
	struct dwc3		*dwc = dep->dwc;
	unsigned long		flags;
...
out:
	...
	if(dwc->irq_gadget){free_irq(dwc->irq_gadget, dwc->ev_buf);}
	if(dwc->irq_wakeup){free_irq(dwc->irq_wakeup, dwc);}
...
static int dwc3_gadget_get_irq(struct dwc3 *dwc)
{
	...
	/* look for wakeup interrupt if hibernation is supported */
	if (dwc->has_hibernation) {
		irq_wakeup = platform_get_irq(dwc3_pdev, 2);
		if (irq_wakeup > 0)
			dwc->irq_wakeup = irq_wakeup;

		if (irq_wakeup == -EPROBE_DEFER){
			irq=-EPROBE_DEFER;
			goto out;
		}
	}
