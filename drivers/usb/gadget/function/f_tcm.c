...
static int bot_send_status(struct usbg_cmd *cmd, bool moved_data)
{
	...
	if (cmd->se_cmd.scsi_status == SAM_STAT_GOOD) {
		...
		//pr_err("scsi err: 0x%x %d\n",cmd->se_cmd.scsi_status,cmd->data_len);
...
static void free_cmd_resource(struct f_uas *fu, struct usb_ep *ep)
{
	...
	for (i = 0; i < fu->ncmd; i++) {
		...
		if(fu->cmd[i]->buf){ kfree(fu->cmd[i]->buf); }
		fu->cmd[i]->buf = NULL;

		if(fu->cmd[i]){ kfree(fu->cmd[i]); }
		fu->cmd[i] = NULL;
	}
}
...
static int usbg_send_status_response(struct se_cmd *se_cmd)
{
	struct usbg_cmd *cmd = container_of(se_cmd, struct usbg_cmd,
			se_cmd);
	struct f_uas *fu = cmd->fu;
	if(cmd->age!=fu->age){return -EINVAL;}
...
static int usbg_send_response(struct usbg_cmd *cmd, int response)
{
	struct f_uas *fu = cmd->fu;
	struct response_iu *iu = (struct response_iu *)&cmd->sense_iu;
	struct uas_stream *stream = cmd->stream;
	...
	iu->tag = cpu_to_be16(cmd->tag);
	iu->response_code = RC_TMF_NOT_SUPPORTED;
	switch(response){
...
static void usbg_tmr_work(struct work_struct *work)
{
	struct usbg_cmd *cmd = container_of(work, struct usbg_cmd, work);
	struct se_cmd *se_cmd;
	struct tcm_usbg_nexus *tv_nexus;
	...
	pr_err("usbg_tmr_work\n");
	se_cmd = &cmd->se_cmd;
	tpg = cmd->fu->tpg;
	tv_nexus = tpg->tpg_nexus;
	//ret = target_submit_tmr(&cmd->se_cmd, tv_nexus->tvn_se_sess,
	//	cmd->sense_iu.sense, cmd->unpacked_lun,
	//	cmd, cmd->tmr_code, GFP_ATOMIC, cmd->task_tag, TARGET_SCF_ACK_KREF);
	//pr_err("target_submit_tmr returns %d\n", ret);
	//if(ret < 0){
	//	usbg_send_response(cmd,TMR_TASK_MGMT_FUNCTION_NOT_SUPPORTED);
	//	transport_generic_free_cmd(&cmd->se_cmd, 0);
	//}else{
	//	return;
	//}
	//target framework seems to have its own bug, just tell win we're good
	//usbg_send_response(cmd,cmd->tmr_code==TMR_LUN_RESET?TMR_FUNCTION_COMPLETE:TMR_TASK_MGMT_FUNCTION_NOT_SUPPORTED);
	usbg_send_response(cmd,TMR_TASK_MGMT_FUNCTION_NOT_SUPPORTED);
}
		...
		void *cmdbuf, unsigned int len)
{
	...
	if (cmd_iu->iu_id == IU_ID_TASK_MGMT) {
		struct task_mgmt_iu *task_iu = cmdbuf;
		int tmr_code = TMR_UNKNOWN;
		if (!tv_nexus) {
			pr_err("Missing nexus, ignoring task management\n");
			return -EINVAL;
		}
		//return 0;
		scsi_tag = be16_to_cpup(&task_iu->tag);
		pr_err("uasp task mgmt %02x, tag=%d\n",task_iu->function,scsi_tag);
		cmd = usbg_get_cmd(fu, tv_nexus, scsi_tag);
		switch(task_iu->function){
		case TMF_ABORT_TASK:
			tmr_code=TMR_ABORT_TASK;
			break;
		case TMF_ABORT_TASK_SET:
			tmr_code=TMR_ABORT_TASK_SET;
			break;
		case TMF_CLEAR_TASK_SET:
			tmr_code=TMR_CLEAR_TASK_SET;
			break;
		case TMF_LOGICAL_UNIT_RESET:
			tmr_code=TMR_LUN_RESET;
			break;
		case TMF_I_T_NEXUS_RESET:
			tmr_code=TMR_TARGET_WARM_RESET;
			//or TMR_TARGET_COLD_RESET?
			break;
		case TMF_CLEAR_ACA:
			tmr_code=TMR_CLEAR_ACA;
			break;
		case TMF_QUERY_TASK:
		case TMF_QUERY_TASK_SET:
		case TMF_QUERY_ASYNC_EVENT:
		default:
			tmr_code=TMR_UNKNOWN;
			break;
		}
		if (fu->flags & USBG_USE_STREAMS) {
			if (cmd->tag > UASP_SS_EP_COMP_NUM_STREAMS){
				pr_err("bad stream %d, set to zero\n",(int)cmd->tag);
				cmd->tag=0;
				//goto err;
			}
			if (!cmd->tag)
		...
		///////////
		cmd->unpacked_lun = scsilun_to_int(&task_iu->lun);
		cmd->task_tag = be16_to_cpup(&task_iu->task_tag);
		cmd->tmr_code = tmr_code;
		INIT_WORK(&cmd->work, usbg_tmr_work);
...
static void bot_cmd_work(struct work_struct *work)
{
	...
	tv_nexus = tpg->tpg_nexus;
	dir = get_cmd_dir(cmd->cmd_buf);
	if(cmd->cmd_buf[0]==MODE_SELECT||cmd->cmd_buf[0]==MODE_SELECT_10){
		//hack for HS - filter out these as in f_mass_storage
		//a1 triggers invalid opcode
		cmd->cmd_buf[0]=0xa1;
		//out: is buggy shit
		//goto out;
	}
...
static void usbg_queue_tm_rsp(struct se_cmd *se_cmd)
{
	struct usbg_cmd *cmd = container_of(se_cmd, struct usbg_cmd,
			se_cmd);
	if(cmd->age!=cmd->fu->age){return;}
	usbg_send_response(cmd,se_cmd->se_tmr_req?se_cmd->se_tmr_req->response:TMR_TASK_MGMT_FUNCTION_NOT_SUPPORTED);
	//what happens if we error again sending response?
}
